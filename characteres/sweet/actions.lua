...
function actions.build()
    ...
    --Atacar usando doces
    local candyAttack ={
        description = "Lançar doces contra o inimigo",
        requirement = nil,
    ...
    -- Lançar bolhas de açúcar para grudar o inimigo
    local bubbleAttack ={
        description = "Lançar bolhas de açúcar",
        requirement = nil,
        execute = function (playerData, creatureData)      
             --1. Calcular dano e apresentar resultado como print
             local rawDamage = creatureData.attack - math.random() * playerData.defense
             local damage = math.min(1, math.ceil(rawDamage * 0.25))
    ...
    actions.list[#actions.list+1] = candyAttack
    actions.list[#actions.list+1] = bubbleAttack
